                          ...
                          const Vector3f& dir, float& tnear, float& u, float& v)
{
    ...
    float t_temp = dotProduct(S2, E2) / denom;
    float u_temp = dotProduct(S1, S) / denom;
    float v_temp = dotProduct(S2, dir) / denom;
    

    if (t_temp >= 0 && u_temp >= 0 && v_temp >= 0 && u_temp <= 1 && v_temp <= 1){
        tnear = t_temp;
        u = u_temp;
        v = v_temp;
        return true;
